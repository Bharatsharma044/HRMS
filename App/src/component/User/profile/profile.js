...
import ModalBox from './editProfileModel.js';
...
const ProfileModule = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${BASE_API_URL}profile/getAll`);
    ...
    const handleInputChange = (e) => {
        ...
        setErrors({
           ...
           const handleSubmit = async (e) => {
            ...
            if (validateForm()) {
                try {
                    const response = await axios.post(`${BASE_API_URL}profile/create`, formData);
                    settogle(!togle)
        ...
        const DeleteData = async (id) => {
            ...
            if (isConfirmed) {
                try {
                    ...
                    const response = await axios.delete(`${BASE_API_URL}profile/delete?id=${id}`);
        ...
        return (
            ...
            <div >
                ...
                <div style={{ backgroundColor: '#28769a' }}>
                    <h1 className='headerData'>WelCome To profile PAGE</h1>
                </div>
                                                ...
                                                <div class="col-md-6 offset-md-3">
                                                    ...
                                                    <form onSubmit={handleSubmit} class="mt-5 border p-4 bg-light shadow">
                                                            <div style={{ textAlign: 'center' }}>
                                                                <h4 style={{ display: 'inline', marginRight: '10px' }} className="mb-5 text-secondary">Add New profile</h4>
                            ...
                            <table className="table">
                                <thead className="thead-light">
                                    <tr>
                                        ...
                                        {/* <th scope="col" onClick={() => handleSort('name')}>Skill {sortColumn === 'name' && (
                                            <FontAwesomeIcon icon={sortDirection === 'ascending' ? faSortUp : faSortDown} />
                                        )}</th> */}
                                        <th scope="col" onClick={() => handleSort('email')}>Profile{sortColumn === 'email' && (
                                            <FontAwesomeIcon icon={sortDirection === 'ascending' ? faSortUp : faSortDown} />
                                        )}</th>
                                        <th scope="col" onClick={() => handleSort('mobile')}> Profile Id {sortColumn === 'mobile' && (
                                            <FontAwesomeIcon icon={sortDirection === 'ascending' ? faSortUp : faSortDown} />
                                        )}</th>
